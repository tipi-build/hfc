...
.. command:: FetchContent_MakeHermetic
  ...
  .. code-block:: cmake
    ...
    FetchContent_MakeHermetic(
      ...
      [HERMETIC_DECLARED_PACKAGE_VERSION <version string>]
    )
  ...
  The ``HERMETIC_DECLARED_PACKAGE_VERSION`` option allows specifying an explicit version string
  for the package. When provided, this version takes priority over any auto-detected version.
  The version is exposed as ``${PackageName}_VERSION`` and ``${PACKAGENAME}_VERSION`` when the
  package is consumed via ``find_package()`` in dependent sub-builds.

  HFC automatically detects version from installed ``*ConfigVersion.cmake`` files and
  pkg-config ``.pc`` files. ``HERMETIC_DECLARED_PACKAGE_VERSION`` is needed for packages that
  do not install any version metadata (e.g. Apache Thrift does not provide a
  ``ConfigVersion.cmake`` or ``.pc`` file).

  .. code-block:: cmake

    FetchContent_MakeHermetic(
      Thrift
      HERMETIC_DECLARED_PACKAGE_VERSION "0.23.0"
