...
add_link_options(-Wl,--build-id)
    ...
    if(fs::exists(openssl_makefile)) {
      ...
      BOOST_REQUIRE_MESSAGE(
        boost::contains(makefile_content, "-Wno-deprecated-declarations"),
        "Toolchain compile option -Wno-deprecated-declarations not found in OpenSSL Makefile (but should be forwarded from toolchain)"
      );
      BOOST_REQUIRE_MESSAGE(
