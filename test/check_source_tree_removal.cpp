...
namespace hfc::test { 
  ...
  BOOST_DATA_TEST_CASE_F(test_isolation_fixture, check_source_tree_removal_for_cmake_deps, boost::unit_test::data::make(hfc::test::test_variants()), data){
    fs::path template_path = prepare_project_to_be_tested("check_compiler_forwarding",data.is_cmake_re, temp_dir);
    ...
    run_command(cmake_configure_command, template_path, test_env);
    BOOST_REQUIRE(!fs::exists(template_path / "build" / "_deps" / "mathlib-src" / "CMakeLists.txt" ));
    auto output = run_command(cmake_build_command, template_path, test_env);
    ...
    run_command(cmake_configure_command, template_path, test_env);
    BOOST_REQUIRE(!fs::exists(template_path / "build" / "_deps" / "mathlib-src" / "CMakeLists.txt" ));
    output = run_command(cmake_build_command, template_path, test_env);
    BOOST_REQUIRE(!boost::contains(output, "ninja: no work to do."));
    BOOST_REQUIRE(!fs::exists(template_path / "build" / "_deps" / "mathlib-src" / "CMakeLists.txt" ));
  }

  BOOST_DATA_TEST_CASE_F(test_isolation_fixture, check_after_source_tree_removal_commit_change_and_config_change_for_cmake_deps, boost::unit_test::data::make(hfc::test::test_variants()), data){
    fs::path template_path = prepare_project_to_be_tested("check_compiler_forwarding",data.is_cmake_re, temp_dir);
    write_simple_main(template_path,{"MathFunctions.h", "MathFunctionscbrt.h"});
    generate_toolchain_with_cmake_re_data((template_path / "toolchain" / "toolchain.cmake"),data.enable_cmake_re);
  ...
  BOOST_DATA_TEST_CASE_F(test_isolation_fixture, check_after_source_tree_removal_commit_change_and_config_change_for_autotools_deps, boost::unit_test::data::make(hfc::test::test_variants()), data){
    fs::path template_path = prepare_project_to_be_tested("check_compiler_forwarding_autotools",data.is_cmake_re, temp_dir);
  ...
  BOOST_DATA_TEST_CASE_F(test_isolation_fixture, check_source_tree_removal_for_autotools_deps, boost::unit_test::data::make(hfc::test::test_variants()), data){
    fs::path template_path = prepare_project_to_be_tested("check_compiler_forwarding_autotools",data.is_cmake_re, temp_dir);
    write_simple_main(template_path,{"lib.h"});
